...
class MembershipsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "show displays stop action and icon-only destroy action with confirmations" do
    ...
    destroy_buttons = css_select("form[action='#{membership_path(membership)}'] button[title='#{I18n.t("active_admin.delete_model")}'][aria-label='#{I18n.t("active_admin.delete_model")}'][data-confirm='#{I18n.t("active_admin.resource.show.destroy_membership_confirm")}']")
