...
class BiddingRound::PledgeTest < ActiveSupport::TestCase
  ...
  test "validates basket_size_price presence" do
    pledge = BiddingRound::Pledge.new(
      bidding_round: bidding_rounds(:open_2024),
      membership: memberships(:jane))

    assert_not pledge.valid?
    assert_includes pledge.errors[:basket_size_price], "can't be blank"
  end
  ...
  test "total_membership_price calculates correctly" do
    membership = memberships(:jane)
    ...
    assert_equal 2 * 10 * 31, pledge.total_membership_price
  end
