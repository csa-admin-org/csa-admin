...
require "test_helper"
require_relative "../../../lib/rails_edge/continuous_integration"

class RailsEdgeContinuousIntegrationTest < ActiveSupport::TestCase
  ...
  test "parallel group falls back to open3 when pty is unavailable" do
    group_class = ActiveSupport::ContinuousIntegration::Group
    original = group_class.instance_method(:pty_available?)
    group_class.define_method(:pty_available?) { false }
