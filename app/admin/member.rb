          ...
          panel link_to(t("shop.title_orders", count: 2), all_orders_path), icon: "shopping-basket", count: orders_count do
            if orders_count.zero?
              ...
              table_for(orders.limit(3), class: "table-auto") do
                column(:id) { |o| auto_link o, o.id, data: { "table-row-action": "show" } }
              ...
              table_for(activity_participations.limit(6), class: "table-auto") do
                column(Activity.model_name.human) { |ap|
                  with_note_icon ap.note, reply: ap.note_reply_args do
                    auto_link ap, ap.activity.name, data: { "table-row-action": "show" }
                  end
                }
        ...
        panel link_to(Payment.model_name.human(count: 2), all_payments_path), icon: "banknotes", count: payments_count do
          if payments_count.zero?
            ...
            table_for(payments.limit(10), class: "table-auto") do
              column(:id) { |p| auto_link p, p.id, data: { "table-row-action": "show" } }
          ...
          attributes_table do
            ...
            row(:next_invoice_on) {
              recurring_billing_row_content(self,
                ...
                membership: invoicer.membership)
            }
          end
        end
