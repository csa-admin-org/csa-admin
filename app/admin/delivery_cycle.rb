...
ActiveAdmin.register DeliveryCycle do
          ...
          panel title, icon: "calendar-days", count: deliveries.count do
            if deliveries.any?
              table_for deliveries, class: "table-auto" do
                ...
                column t("attributes.date"), ->(d) {
                  auto_link d, l(d.date, format: :day_month), data: { "table-row-action": "show" }
                }, class: "text-right"
      ...
      column do
        ...
        panel t(".billing"), icon: "banknotes" do
          attributes_table do
            ...
            if feature?("absence")
              row(:absences_included_annually) { dc.absences_included_annually }
            end
          end
        end
        ...
        translated_input(f, :form_details,
          ...
          placeholder: ->(locale) {
            I18n.with_locale(locale) {
              delivery_cycle_details(form_details, force_default: true)
            }
          })
  ...
  collection_action :form_details_preview, method: :get do
    authorize! :read, DeliveryCycle
    placeholders = helpers.form_details_preview_placeholders(
      helpers.form_details_preview_delivery_cycle(params[:delivery_cycle]),
      :delivery_cycle_details)
    render html: helpers.form_details_preview_frame("delivery-cycle", placeholders)
  end
