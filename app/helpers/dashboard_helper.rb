    ...
    content_tag :div, class: "flex items-center space-x-3" do
      (
        if latest_snapshots.any?
          ...
          } + content_tag(:div, id: "billing-snapshots-menu", class: "z-50 hidden min-w-max bg-white rounded-sm shadow-lg ring-1 ring-black/5 focus:outline-hidden dark:bg-gray-800 py-2 px-2 text-right text-sm text-gray-700 dark:text-gray-200", "aria-labelledby" => "user-menu-button") do
